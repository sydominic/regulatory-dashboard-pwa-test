...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$r=Invoke-RestMethod -Uri '%API_HEALTH%' -TimeoutSec 5; if ($r.apiVersion -ne 'v1.1-node-render-async-diagnostic') { Write-Output ('ERROR: expected v1.1-node-render-async-diagnostic but got ' + $r.apiVersion); exit 1 } else { Write-Output ('API version OK: ' + $r.apiVersion); exit 0 }" >> "%RUN_LOG%" 2>&1
if errorlevel 1 (
